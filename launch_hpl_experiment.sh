...
nores=0
cruntime=enroot
container=nvidia+hpc-benchmarks+23.5.sqsh
#container=nvidia+hpc-benchmarks+21.4-hpl.sqsh
#container=nvidia+hpc-benchmarks+20.10-hpl.sqsh 
ORDER_CMD="cat"
slurmopts=""
...
Other Options:
    ...
    -n|--name experiment.  If nothing is input, the default is <nodes_per_job>node_<system>_date +%Y%m%d%H%M%S
...
if [ x"${system}" == x"dgxa100_40G" ]; then
...
if [ x"${nameexp}" == x"" ]; then
    ...
    export EXPNAME=${nameexp}_${nodes_per_job}node_${system}_$(date +%Y%m%d)
fi
...
for V in HPL_DIR HPL_SCRIPTS_DIR EXPDIR system cruntime CONT nodes_per_job gpus_per_node niters partition slurmopts maxnodes mpiopts gresstr total_nodes hpldat hplai nameexp; do
    echo -n "${V}: "
    if [ x"${!V}" != x"" ]; then	
        echo "${!V}"
    else
...
for N in $(seq ${niters}); do
    ...
    while [ $(( P + nodes_per_job - 1 ))  -le ${total_nodes} ]; do
        ...
        CMD="sbatch -J case-${INST} -N ${nodes_per_job} "
